...
class Model(object):
    ...
    def __repr__(self):
        ...
        '''
        text = '{} ~ {} + '.format(np.array(self.dataset.labels)[self.dep_var],
                               round(self.beta[0], 4))
        for i in range(len(self.beta) - 1):
            text += '({} * {})'.format(round(self.beta[i + 1], 4),
                                       self.dataset.labels[self.pred_vars[i]])
            if i != len(self.beta) - 2:
                text += " + "
        return text

    def X(self, train=True):
        '''
        Computes the X matrix for R2 calculation based on training or testing
        data.
        '''
        data = self.dataset.testing_data
        if train:
            data = self.dataset.training_data
        return util.prepend_ones_column(data[:, self.pred_vars])

    def y(self, train=True):
        '''
        Computes the y vector for R2 calculation based on training or testing
        data.
        '''
        data = self.dataset.testing_data
        if train:
            data = self.dataset.training_data
        return data[:, self.dep_var]

    def calc_beta(self):
        '''
        Computes an array of betas for lineer regression equation.
        '''
        return util.linear_regression(self.X(), self.y())

    def y_hat(self, train=True):
        '''
        Performs martix multiplication to calculate y_hat.
        '''
        return np.matmul(self.X(train), self.calc_beta())

    def calc_R2(self, train=True):
        '''
        Calculates R2 score based on training or testing data.
        '''
        return  (1 - (np.sum((self.y(train) - self.y_hat(train)) ** 2) /
                    np.sum((self.y(train) - np.mean(self.y(train))) ** 2)))
...
def compute_best_pair(dataset):
    ...
    '''
    max_R2 = 0
    for x in dataset.pred_vars:
        for i in dataset.pred_vars:
            if x < i:
                if Model(dataset, [x, i]).R2 > max_R2:
                    max_R2 = Model(dataset, [x, i]).R2
                    best_pair = [x, i]

    return Model(dataset, best_pair)
...
def forward_selection(dataset):
    ...
    '''
    max_R2 = 0
    var_lst = dataset.pred_vars.copy()
    fwd_list = [[]]
    for _ in range(len(var_lst)):
        for var in var_lst:
            if Model(dataset, fwd_list[-1] + [var]).R2 > max_R2:
                max_R2 = Model(dataset, fwd_list[-1] + [var]).R2
                best_var = var
        fwd_list += [fwd_list[-1] + [best_var]]
        var_lst.remove(best_var)
    return [Model(dataset, i) for i in fwd_list if i != []]
